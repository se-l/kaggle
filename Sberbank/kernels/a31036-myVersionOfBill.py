...
def scorecv(xgbparams, saveModel=False):
    ...
    for train_index, test_index in skf.split(x_train, y_train):
        ...
        xgbMTest = xgb.DMatrix(x_testF, label=y_testF)#, feature_names=feature_names)
...
def logCVout(batch,xgb_params, num_boost_rounds, res):
    Logger.info('Opt{} CV par: {}  numboost: {}\nresult: {}'.format(
        batch,xgb_params, num_boost_rounds, res))
...
if hyperP.loadModel == 0:
    ...
    xgbFunc = partial(scorecv, xgbparams=xgb_params)
    xgb_params, num_boost_rounds, res = optimize(trials1, xgb_params, scorecv)#xgbFunc)
    logCVout(1,xgb_params, num_boost_rounds, res)
...
if hyperP.loadModel == 0:
    ...
    trials2 = Trials()
    # xgb_paramsT, num_boost_roundsT = optimize(trials1, space, scorecv)
    xgb_params, num_boost_rounds, res = optimize(trials2, xgb_params, scorecv)
    logCVout(2, xgb_params, num_boost_rounds, res)
    # pickle.dump(trials1, open(os.path.join(projectDir, 'hyperOptTrials/{}.Sberbanktrial1'.format(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))),'wb'))
    xgb_args = {
        'num_boost_round': math.ceil(num_boost_rounds * hyperP.boostMulti),
    }
...
if hyperP.loadModel == 0:
    ...
    trials3 = Trials()
    # xgb_paramsT, num_boost_roundsT = optimize(trials1, space, scorecv)
    xgb_params, num_boost_rounds, res = optimize(trials3, xgb_params, scorecv)
    logCVout(3, xgb_params, num_boost_rounds, res)
    # pickle.dump(trials1, open(os.path.join(projectDir, 'hyperOptTrials/{}.Sberbanktrial1'.format(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))),'wb'))
    xgb_args = {
        'num_boost_round': math.ceil(num_boost_rounds * hyperP.boostMulti),
    }
